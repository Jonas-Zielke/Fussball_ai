"""Train V3 ensemble of 3 models (smaller, faster)."""
import json
import numpy as np
import pandas as pd
import torch
...
# 3 similar configs (kleiner, schneller, gleicher hidden size für einfacheres JS)
configs = [
    {"seed": 42, "hidden": 96, "n_blocks": 4, "dropout": 0.20},
    {"seed": 13, "hidden": 96, "n_blocks": 5, "dropout": 0.25},
    {"seed": 23, "hidden": 96, "n_blocks": 4, "dropout": 0.30},
]
...
for i, sd in enumerate(models_serialized):
    ...
    new_sd = {
        "input_proj.0.weight": torch.tensor(sd["input_w"]),
        "input_proj.0.bias": torch.tensor(sd["input_b"]),
    }
...
# Save with FIXED architectures (all same n_blocks for simpler JS)
bundle = {
    "architecture": {
        "n_models": len(models_serialized),
        "in_dim": in_dim,
        "hidden": 96,
        "n_blocks": 4,  # all 3 models have 4 blocks now
        "n_classes": 3,
        "version": "v3-ensemble-3",
    },
